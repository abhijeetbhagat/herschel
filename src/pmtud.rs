...
impl Pmtud {
    pub fn new(destination: Ipv4Addr) -> Result<Self, PmtudError> {
        let (tx, rx) = transport_channel(1500, TransportChannelType::Layer3(Ipv4))
    ...
    pub fn discover(&mut self) -> u8 {
        ...
        packet.set_source("127.0.0.1".parse().unwrap()); // nats can change this address
        ...
        match self.tx.send_to(packet, self.destination.into()) {
            ...
            _ => while let Ok((packet, addr)) = ipv4_packet_iter(&mut self.rx).next() {},
        }
        todo!()
    }
}
